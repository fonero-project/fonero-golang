...
func TestValidateError(t *testing.T) {
	...
	authData := &AuthData{
		Sender:         "bartekfonero.org",
		NeedInfo:       false,
		Tx:             "&^%",
		AttachmentJSON: "abc",
	}

	assert.EqualError(t, authData.Validate(), "sender: bartekfonero.org does not validate as fonero_address;tx: &^% does not validate as base64;attachment: abc does not validate as json")
}
