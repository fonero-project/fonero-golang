...
type AuthData struct {
	// The fonero address of the customer that is initiating the send.
	Sender string `json:"sender" valid:"required,fonero_address"`
...
type AuthResponse struct {
	...
	// (only present if info_status is ok) JSON of the recipient's AML information. in the Fonero attachment convention
...
// Transaction represents transaction field in Fonero attachment
...
// Operation represents a single operation object in Fonero attachment
...
const (
	...
	// tx_status endpoint response. It represents that the
	// payment was approved by the receiving FI but the Fonero
	// transaction hasn't been received yet
	TransactionStatusApproved TransactionStatus = "approved"

	// TransactionStatusNotApproved is a value of `status` field for the
	// tx_status endpoint response. It represents that the
	// Fonero transaction was found but it was never approved
	// by the receiving FI.
