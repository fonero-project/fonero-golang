...
then
    ...
    git push --delete -q https://$GITHUB_OAUTH_TOKEN@github.com/fonero-project/fonero-golang snapshots || true
    git tag snapshots -a -m "Generated snapshots from TravisCI for build $TRAVIS_BUILD_NUMBER on $(date --utc +'%F-%T')"
    git push -q https://$GITHUB_OAUTH_TOKEN@github.com/fonero-project/fonero-golang --tags
fi
