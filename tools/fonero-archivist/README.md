...
This is a small tool, written in Go, for working with `fonero-core` history archives directly.
It is a standalone tool that does not require `fonero-core`, or any other programs.

It is much smaller and simpler than `fonero-core`, and is intended only for archive-maintenance tasks.
...
Unlike `fonero-core`, `fonero-archivist` does not run subprocesses to access history archives;
...
`fonero-archivist` supports reading from and writing to any S3-compatible storage.
