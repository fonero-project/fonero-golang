...
Bifrost is highly available and secure Bitcoin/Ethereum → Fonero bridge. It allows users to move BTC/ETH to Fonero network and then trade them to other tokens or participate in ICOs (initial coin offering).

It solves many problems connected to moving tokens to Fonero network:

* Security:
  * Developers don’t have access to users’ Fonero keys.
...
Download the binary from [the release page](https://github.com/fonero-project/fonero-golang/releases/tag/bifrost-v0.0.2) and use it with it's [Bifrost JS SDK](https://github.com/fonero-project/bifrost-js-sdk).

## How it works

1. User opens the web app implemented using [Bifrost JS SDK](https://github.com/fonero-project/bifrost-js-sdk).
1. User is presented with her public and private Fonero keys where Bitcoin/Ethereum will be sent.
1. User selects what cryptocurrency she wants to move to Fonero network.
1. A receiving Bitcoin/Ethereum address is generated.
1. User sends funds in Bitcoin/Ethereum network.
1. Bifrost listens to Bitcoin and Ethereum network events. When payment arrives it creates a Fonero [account](https://www.fonero.org/developers/guides/concepts/accounts.html) for the user who later adds a temporary signer on the account.
1. Using a temporary signer Bifrost creates necessary [trust lines](https://www.fonero.org/developers/guides/concepts/assets.html) and sends corresponding amount of BTC/ETH to user's Fonero account. It also exchanges BTC/ETH to the final token at a given rate.
...
1. Now open Bifrost demo: https://bifrost.fonero.org/ It will display an Ethereum address for sending your test ETH.
...
* `bitcoin`
  ...
  * `token_price` - a price of one BTC in terms of final token (`fonero.token_asset_code`)
* `ethereum`
  ...
  * `token_price` - a price of one ETH in terms of final token (`fonero.token_asset_code`)
* `fonero`
  ...
  * `signer_secret_key` - distribuions accounts's secret key if only one instance of Bifrost is deployed OR [channel](https://www.fonero.org/developers/guides/channels.html)'s secret key if more than one instance of Bifrost is deployed, signer is also used as a temporary signer in new accounts (see "Account configuration" section)
  * `horizon` - URL to [horizon](https://github.com/fonero-project/fonero-golang/tree/master/services/horizon) server
  * `network_passphrase` - Fonero network passphrase (`Public Global Fonero Network ; September 2015` for production network, `Test SDF Network ; September 2015` for test network)
  * `starting_balance` - Fonero FNO amount issued to created account (41 by default)
...
This is the most basic configuration but it is not recommended. In this configuration only one account is responsible for everything: issuing and distributing assets and creating new accounts. In this configuration `fonero.issuer_public_key`, `fonero.distribution_public_key` and `fonero.signer_secret_key` represent a single account.

### Issuing and Distribution Accounts

Sometimes you want to have a limited supply of your token so you create a distribution account, send all the tokens to distribution (and other accounts, ex. vesting) and lock the issuing account so no new tokens can be created. In this configuration `fonero.distribution_public_key` and `fonero.signer_secret_key` represent a separate account used to distribute funds in a token sale. This account will also be used to create new accounts.

### Distribution Account with multiple signers

If you expect really high load you should deploy multiple Bifrost servers. To prevent Bifrost instances from consuming the same sequence numbers of the Distribution account you should use [channels](https://www.fonero.org/developers/guides/channels.html). Each Bifrost server will contain a secret key of one of the signers of Distribution account and will use Signer account as a [transaction source](https://www.fonero.org/developers/guides/concepts/transactions.html#source-account) when creating new accounts.

In this configuration all servers have the same `fonero.issuer_public_key` and `fonero.distribution_public_key` values but each server has a separate `fonero.signer_secret_key` which is a signer of `fonero.distribution_public_key` account (and needs to exist in Fonero ledger).
...
* Make sure "Sell [your token] for BTC" and/or "Sell [your token] for ETH" exist in Fonero production network. If not, you can create an offer by sending a transaction with `manage_offer` operation.
* Make sure you don't use account from `fonero.issuer_secret_key` anywhere else than bifrost. Otherwise, sequence numbers will go out of sync and bifrost will stop working. It's good idea to create a new signer on issuing account.
...
* Deploy at least 2 bifrost, bitcoin-core, geth, fonero-core and horizon servers. Use multi-AZ database.
...
* It's a good idea to set "Authorization required" and "Authorization revocable" [flags](https://www.fonero.org/developers/guides/concepts/accounts.html#flags) on issuing account during ICO stage to remove trustlines to accounts with lost keys. You need to set `fonero.needs_authorize` in config to `true` if you want to use this feature.
...
* `Bifrost.ExchangedTimelockedEvent` [event](https://github.com/fonero-project/bifrost-js-sdk#oneventevent-data). You can display it to the user if you want your users to unlock accounts.
...
Check [Fonero Stack Exchange](https://fonero.stackexchange.com/questions/tagged/bifrost/) to find similar questions or add the new question with the `bifrost` tag.
