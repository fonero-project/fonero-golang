...
type Config struct {
	...
	Fonero struct {
		...
		IssuerPublicKey string `valid:"required,fonero_accountid" toml:"issuer_public_key"`
		...
		DistributionPublicKey string `valid:"required,fonero_accountid" toml:"distribution_public_key"`
		// SignerSecretKey is:
		...
		// https://www.fonero.org/developers/guides/channels.html
		// Signer's sequence number will be consumed in transaction's sequence number.
		SignerSecretKey string `valid:"required,fonero_seed" toml:"signer_secret_key"`
		// StartingBalance is the starting amount of FNO for newly created accounts.
		// Default value is 41. Increase it if you need Data records / other custom entities on new account.
		StartingBalance string `valid:"optional,fonero_amount" toml:"starting_balance"`
...
func (c Config) SignerPublicKey() string {
	kp := keypair.MustParse(c.Fonero.SignerSecretKey)
	return kp.Address()
}
