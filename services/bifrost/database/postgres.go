...
func fromQueueTransaction(tx queue.Transaction) *transactionsQueueRow {
	return &transactionsQueueRow{
		...
		Amount:           tx.Amount,
		FoneroPublicKey: tx.FoneroPublicKey,
	}
}
...
func (r *transactionsQueueRow) toQueueTransaction() *queue.Transaction {
	return &queue.Transaction{
		...
		Amount:           r.Amount,
		FoneroPublicKey: r.FoneroPublicKey,
	}
}
...
func (d *PostgresDatabase) CreateAddressAssociation(chain Chain, foneroAddress, address string, addressIndex uint32) error {
...
func (d *PostgresDatabase) GetAssociationByFoneroPublicKey(foneroPublicKey string) (*AddressAssociation, error) {
	addressAssociationTable := d.getTable(addressAssociationTableName, nil)
	row := &AddressAssociation{}
	where := map[string]interface{}{"fonero_public_key": foneroPublicKey}
	err := addressAssociationTable.Get(row, where).Exec()
	if err != nil {
		switch errors.Cause(err) {
		case sql.ErrNoRows:
