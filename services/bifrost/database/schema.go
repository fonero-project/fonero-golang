...
CREATE TABLE address_association (
  ...
  fonero_public_key varchar(56) NOT NULL UNIQUE,
  created_at timestamp NOT NULL,
  PRIMARY KEY (chain, address_index, address, fonero_public_key),
...
CREATE TABLE transactions_queue (
  ...
  /* ethereum: 100000000 in year 2128 + 7 decimal precision in Fonero + dot */
  /* bitcoin:   21000000              + 7 decimal precision in Fonero + dot */
  amount varchar(20) NOT NULL,
  fonero_public_key varchar(56) NOT NULL,
  ...
  CONSTRAINT valid_fonero_public_key CHECK (char_length(fonero_public_key) = 56)
);
