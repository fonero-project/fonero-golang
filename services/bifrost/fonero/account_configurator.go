...
func (ac *AccountConfigurator) Start() error {
	ac.log = common.CreateLogger("FoneroAccountConfigurator")
	ac.log.Info("FoneroAccountConfigurator starting")
...
func (ac *AccountConfigurator) ConfigureAccount(destination, assetCode, amount string) {
	...
	localLog.Info("Configuring Fonero account")
	...
	for {
		...
		localLog.WithField("destination", destination).Info("Creating Fonero account")
		err = ac.createAccountTransaction(destination)
		if err != nil {
			localLog.WithField("err", err).Error("Error creating Fonero account")
			time.Sleep(2 * time.Second)
			continue
		}
