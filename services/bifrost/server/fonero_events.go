...
func (s *Server) onFoneroAccountCreated(destination string) {
	association, err := s.Database.GetAssociationByFoneroPublicKey(destination)
	if err != nil {
	...
	if association == nil {
		s.log.WithField("foneroPublicKey", destination).Error("Association not found")
		return
	}
