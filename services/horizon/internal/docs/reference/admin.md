...
Horizon is dependent upon a fonero-core server.  Horizon needs access to both the SQL database and the HTTP API that is published by fonero-core. See [the administration guide](https://www.fonero.org/developers/fonero-core/learn/admin.html
) to learn how to set up and administer a fonero-core server.  Secondly, horizon is dependent upon a postgres server, which it uses to store processed core data for ease of use. Horizon requires postgres version >= 9.3.
...
1. Clone horizon's source:  `go get github.com/fonero-project/fonero-golang && cd $GOPATH/src/github.com/fonero-project/fonero-golang/`
2. Under the project folder, download external dependencies: `glide install`
3. Build the binary: `go install github.com/fonero-project/fonero-golang/services/horizon`
