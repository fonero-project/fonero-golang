# bridge-server
This is a stand alone server written in go. It is designed to make connecting to the Fonero network as easy as possible. 
...
- Creating Fonero transactions.
- Monitoring a receiving Fonero account.
...
The `bridge.cfg` file must be present in a working directory (you can load another file by using `-c` parameter). Here is an [example configuration file](https://github.com/fonero-project/bridge-server/blob/master/bridge_example.cfg). Config file should contain following values:
...
* `horizon` - URL to [horizon](https://github.com/fonero-project/horizon) server instance
* `assets` - array of approved assets codes that this server can authorize or receive. These are currency code/issuer pairs. Use asset code 'FNO' with no issuer to listen for FNO payments. See [`bridge_example.cfg`](./bridge_example.cfg) for example.
* `database`
...
* `mac_key` - a fonero secret key used to add MAC headers to a payment notification.
...
* [`InternalServerError`](/src/github.com/fonero-project/gateway/protocols/errors.go)

### POST /builder

Builds a transaction from a given request. `Content-Type` of this request should be `application/json`. Check [List of operations](https://www.fonero.org/developers/learn/concepts/list-of-operations.html) doc to learn more about how each operation looks like.

**Note** This will not submit a transaction to the network. Please use [Horizon](https://www.fonero.org/developers/horizon/reference/endpoints/transactions-create.html) to submit a transaction.
...
Assets are represented by a JSON object with two fields: `code` and `issuer`. Empty JSON object represents [native asset](https://www.fonero.org/developers/learn/concepts/assets.html#foneros-fno-).
...
* [`InternalServerError`](/src/github.com/fonero-project/gateway/protocols/errors.go)
* [`InvalidParameterError`](/src/github.com/fonero-project/gateway/protocols/errors.go)

### POST /payment

Builds and submits a transaction with a single [`payment`](https://www.fonero.org/developers/learn/concepts/list-of-operations.html#payment), [`path_payment`](https://www.fonero.org/developers/learn/concepts/list-of-operations.html#path-payment) or [`create_account`](https://www.fonero.org/developers/learn/concepts/list-of-operations.html#create-account) (when sending native asset to account that does not exist) operation built from following parameters.

#### Safe transaction resubmittion

It’s possible that you will not receive a response from Bridge server due to a bug, network conditions, etc. In such situation it’s impossible to determine the status of your transaction and sending the same request to the Bridge server may result in "double-spend" of the funds. That’s why you should always send a request with `id` parameter set. Then when you resubmit a transaction with the same `id` the previously used [sequence number](https://www.fonero.org/developers/guides/concepts/transactions.html) will be reused.
...
`sender` | optional | Payment address (ex. `bob*fonero.org`) of payment sender account. Required for when sending using Compliance protocol.
`destination` | required | Account ID or payment address (ex. `bob*fonero.org`) of payment destination account
...
`extra_memo` | optional | You can include any info here and it will be included in the pre-image of the transaction's memo hash. See the [Fonero Memo Convention](https://github.com/fonero-project/fonero-protocol/issues/28). When set and compliance server is connected, `memo` and `memo_type` values will be ignored.
...
`path[n+1][asset_code]` | optional | [path_payment] Asset code of `n+1`th asset on the path (FNO when empty, but empty parameter must be sent!)
...
```
forward_destination[domain]=fonero.org&forward_destination[fields][forward_type]=bank_account&forward_destination[fields][swift]=BOPBPHMM&forward_destination[fields][acct]=2382376
```

will be translate to the following request:

```
https://FEDERATION_SERVER_READ_FROM_FONERO_TOML/federation?type=forward&forward_type=bank_account&swift=BOPBPHMM&acct=2382376
```

#### Response

It will return [`SubmitTransactionResponse`](/src/github.com/fonero-project/gateway/horizon/submit_transaction_response.go) if there were no errors or with one of the following errors:

* [`InternalServerError`](/src/github.com/fonero-project/gateway/protocols/errors.go)
* [`InvalidParameterError`](/src/github.com/fonero-project/gateway/protocols/errors.go)
* [`MissingParameterError`](/src/github.com/fonero-project/gateway/protocols/errors.go)
* [`TransactionBadSequence`](/src/github.com/fonero-project/gateway/protocols/bridge/errors.go)
* [`TransactionBadAuth`](/src/github.com/fonero-project/gateway/protocols/bridge/errors.go)
* [`TransactionInsufficientBalance`](/src/github.com/fonero-project/gateway/protocols/bridge/errors.go)
* [`TransactionNoAccount`](/src/github.com/fonero-project/gateway/protocols/bridge/errors.go)
* [`TransactionInsufficientFee`](/src/github.com/fonero-project/gateway/protocols/bridge/errors.go)
* [`TransactionBadAuthExtra`](/src/github.com/fonero-project/gateway/protocols/bridge/errors.go)
* [`PaymentCannotResolveDestination`](/src/github.com/fonero-project/gateway/protocols/bridge/payment.go)
* [`PaymentCannotUseMemo`](/src/github.com/fonero-project/gateway/protocols/bridge/payment.go)
* [`PaymentSourceNotExist`](/src/github.com/fonero-project/gateway/protocols/bridge/payment.go)
* [`PaymentAssetCodeNotAllowed`](/src/github.com/fonero-project/gateway/protocols/bridge/payment.go)
* [`PaymentPending`](/src/github.com/fonero-project/gateway/protocols/bridge/payment.go)
* [`PaymentDenied`](/src/github.com/fonero-project/gateway/protocols/bridge/payment.go)
* [`PaymentMalformed`](/src/github.com/fonero-project/gateway/protocols/bridge/payment.go)
* [`PaymentUnderfunded`](/src/github.com/fonero-project/gateway/protocols/bridge/payment.go)
* [`PaymentSrcNoTrust`](/src/github.com/fonero-project/gateway/protocols/bridge/payment.go)
* [`PaymentSrcNotAuthorized`](/src/github.com/fonero-project/gateway/protocols/bridge/payment.go)
* [`PaymentNoDestination`](/src/github.com/fonero-project/gateway/protocols/bridge/payment.go)
* [`PaymentNoTrust`](/src/github.com/fonero-project/gateway/protocols/bridge/payment.go)
* [`PaymentNotAuthorized`](/src/github.com/fonero-project/gateway/protocols/bridge/payment.go)
* [`PaymentLineFull`](/src/github.com/fonero-project/gateway/protocols/bridge/payment.go)
* [`PaymentNoIssuer`](/src/github.com/fonero-project/gateway/protocols/bridge/payment.go)
* [`PaymentTooFewOffers`](/src/github.com/fonero-project/gateway/protocols/bridge/payment.go)
* [`PaymentOfferCrossSelf`](/src/github.com/fonero-project/gateway/protocols/bridge/payment.go)
* [`PaymentOverSendmax`](/src/github.com/fonero-project/gateway/protocols/bridge/payment.go)
...
It will build and submits a transaction with a [`allow_trust`](https://www.fonero.org/developers/learn/concepts/list-of-operations.html#allow-trust) operation. 
The source of this transaction will be the account specified by `accounts.authorizing_seed` config parameter. 
You should make sure that this account is a low weight signer on the issuing account. See [Multi-sig](https://www.fonero.org/developers/learn/concepts/multi-sig.html) for more information. 
...
* [`TransactionInsufficientFee`](/src/github.com/fonero-project/gateway/protocols/bridge/errors.go)
* [`TransactionBadAuthExtra`](/src/github.com/fonero-project/gateway/protocols/bridge/errors.go)
* [`AllowTrustMalformed`](/src/github.com/fonero-project/gateway/protocols/bridge/authorize.go)
* [`AllowTrustNoTrustline`](/src/github.com/fonero-project/gateway/protocols/bridge/authorize.go)
* [`AllowTrustTrustNotRequired`](/src/github.com/fonero-project/gateway/protocols/bridge/authorize.go)
* [`AllowTrustCantRevoke`](/src/github.com/fonero-project/gateway/protocols/bridge/authorize.go)
...
`data` | Value of the [AuthData](https://www.fonero.org/developers/learn/integration-guides/compliance-protocol.html). This field will be empty when compliance server is not connected.
