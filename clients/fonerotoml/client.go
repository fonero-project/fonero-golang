...
func (c *Client) GetFoneroToml(domain string) (resp *Response, err error) {
	...
	limitReader := io.LimitReader(hresp.Body, FoneroTomlMaxSize)
	_, err = toml.DecodeReader(limitReader, &resp)

	// There is one corner case not handled here: response is exactly
	// FoneroTomlMaxSize long and is incorrect toml. Check discussion:
	// https://github.com/fonero-project/fonero-golang/pull/24#discussion_r89909696
	if err != nil && limitReader.(*io.LimitedReader).N == 0 {
		err = errors.Errorf("fonero.toml response exceeds %d bytes limit", FoneroTomlMaxSize)
		return
	}
...
func (c *Client) GetFoneroTomlByAddress(addy string) (*Response, error) {
	...
	return c.GetFoneroToml(domain)
}

// url returns the appropriate url to load for resolving domain's fonero.toml
// file
